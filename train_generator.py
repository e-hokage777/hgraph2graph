...
for epoch in range(args.epoch):
    ...
    for batch in tqdm(dataset):
        ...
        meters = meters + np.array(
            [
                kl_div.detach().cpu().item() if hasattr(kl_div, "detach") else kl_div,
                loss.item(),
                wacc.item() * 100,
                iacc.item() * 100,
                tacc.item() * 100,
        ...
        # kl_val = kl_div.item() if hasattr(kl_div, "item") else kl_div
        # loss_val = loss.item() if hasattr(loss, "item") else loss
        # meters = meters + np.array(
        #     [kl_val, loss_val, wacc * 100, iacc * 100, tacc * 100, sacc * 100]
        # )
